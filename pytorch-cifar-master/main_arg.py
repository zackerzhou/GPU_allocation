...
# parameter transfer using "argv"
# [epoch,batch,train/test,]

# print('length of argv list is',len(sys.argv))
...
parser.add_argument('--epoch', required=True, help='epoch to train or test')
parser.add_argument('--batch', required=True, help='batch to train or test')
parser.add_argument('--job', required=True, help='mode to task')
parser.add_argument('--net', required=True, help='name to net')
...
# print(sys.argv)
# no_epoch = int(sys.argv[1])
# no_batch = int(sys.argv[2])
# job = sys.argv[3]
# name_net = sys.argv[4]
# print("epoch:%d, batch:%d, job:%s, net:%s" %(no_epoch,no_batch,job,name_net))

no_epoch = int(args.epoch)
no_batch = int(args.batch)
job = args.job
name_net = args.net
# exit(0)
# ***********************
...
trainloader = torch.utils.data.DataLoader(trainset, batch_size=no_batch, shuffle=True, num_workers=2)

testset = torchvision.datasets.CIFAR10(root='./data', train=False, download=True, transform=transform_test)
# testloader = torch.utils.data.DataLoader(testset, batch_size=100, shuffle=False, num_workers=2)
# testloader = torch.utils.data.DataLoader(testset, batch_size=32, shuffle=False, num_workers=2)
# testloader = torch.utils.data.DataLoader(testset, batch_size=128, shuffle=False, num_workers=2)
testloader = torch.utils.data.DataLoader(testset, batch_size=no_batch, shuffle=False, num_workers=2)
...
if name_net == 'VGG':
    ...
    net = ResNet18()
elif name_net == 'GoogleNet':
    net = GoogLeNet()
elif name_net == 'DenseNet':
    net = DenseNet121()
elif name_net == 'MobileNet':
    net = MobileNet()
